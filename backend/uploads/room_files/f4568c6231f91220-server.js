...
app.post('/api/rooms/:roomId/join', authenticateToken, async (req, res) => {
  try {
    ...
    // Get user details for the participant info
    const user = await User.findById(req.user.id).select('name profilePic');
    
    // Return room details along with the user's info
    res.json({
      room,
      participant: {
        id: user._id,
...
app.post('/api/rooms/:roomId/files', authenticateToken, multer().single('file'), async (req, res) => {
  try {
    ...
    // Save file to disk if it exists
    if (req.file) {
...
// Modify the Socket.IO implementation
...
io.on('connection', (socket) => {
  ...
  socket.on('newFile', (file) => io.emit('newFile', file));
  socket.on('disconnect', () => console.log('User disconnected'));
});
server.listen(6000, () => console.log('Socket.IO on port 6000'));